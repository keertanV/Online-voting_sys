...
def AdminHome(root,frame1,frame3):
    ...
    runServer = Button(frame1, text="Run Server", width=50, command = lambda: sb_p.call('start python Server.py', shell=True))

    registerVoter = Button(frame1, text="Register Voter", width=50, command = lambda: regV.Register(root, frame1))

    showVotes = Button(frame1, text="Show Votes", width=50, command = lambda: adFunc.showVotes(root, frame1))

    reset = Button(frame1, text="Reset All", width=50, command = lambda: adFunc.resetAll(root, frame1))
...
def log_admin(root,frame1,admin_ID,password):
    ...
    if(admin_ID=="keertan" and password=="123"):
...
def AdmLogin(root,frame1):
    ...
    Label(frame1, text="Admin Login", font=('Helvetica', 50, 'bold')).grid(row = 0, column = 2, rowspan=1)
    Label(frame1, text="").grid(row = 1,column = 0)
