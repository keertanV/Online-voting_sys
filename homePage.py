...
def Home(root, frame1, frame2):
    ...
    Label(frame1, text="Home", font=('Helvetica', 100, 'bold')).grid(row=0, column=1, rowspan=1)
    Label(frame1, text="").grid(row=1, column=0)

    admin = Button(frame1, text="Admin Login", width=55, command=lambda: AdmLogin(root, frame1))

    voter = Button(frame1, text="Voter Login", width=55, command=lambda: voterLogin(root, frame1))

    newTab = Button(frame1, text="New Window", width=55, command=lambda: sb_p.call('start python homePage.py', shell=True))
    ...
    img = Image.open("img\\background_image.png")  
    img = img.resize((600, 1000), Image.BICUBIC)
